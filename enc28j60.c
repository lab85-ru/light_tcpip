...
// init gpio interupts --begin --------------------------------------------

  	/* Enable the BUTTON Clock */
//  	RCC_APB2PeriphClockCmd(ETH_INT_GPIO_CLK | RCC_APB2Periph_AFIO, ENABLE);

  	/* Configure Button pin as input floating */
//  	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
//  	GPIO_InitStructure.GPIO_Pin = ETH_INT_PIN;
//  	GPIO_Init(ETH_INT_GPIO_PORT, &GPIO_InitStructure);

  	/* Connect Button EXTI Line to Button GPIO Pin */
//  	GPIO_EXTILineConfig(ETH_INT_EXTI_PORT_SOURCE, ETH_INT_EXTI_PIN_SOURCE);

////  EXTI_ClearITPendingBit(EXT_UART_EXTI_LINE[uart_num]);

  	/* Configure Button EXTI line */
//  	EXTI_InitStructure.EXTI_Line = ETH_INT_EXTI_LINE;
...
//  	NVIC_InitStructure.NVIC_IRQChannel = EXTI9_5_IRQn;///EXT_UART_IRQn[uart_num];
//  	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0xf;//f;//f
//  	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0x0;
//  	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;

//  	NVIC_Init(&NVIC_InitStructure);

// init gpio interupts --end --------------------------------------------
	...
	enc28j60_release();
	 for (i=0; i<720000; i++) {
		 GPIO_ResetBits(GPIOB, GPIO_Pin_5);//A 7
	 }
	 GPIO_SetBits(GPIOB, GPIO_Pin_5);//A 7
